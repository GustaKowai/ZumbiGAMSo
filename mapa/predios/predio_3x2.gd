...
func _ready() -> void:
	if randi_range(0,1) == 0:
		...
		predio_shape_collision.shape.set_size(Vector2(148, 309.0))
		predio_shape_collision.position.y = -280.5
	else:
		predio_sprite.texture = sprites_alturas[1]
		predio_shape_collision.shape.set_size(Vector2(148, 275.0))
		predio_shape_collision.position.y = -221.5
