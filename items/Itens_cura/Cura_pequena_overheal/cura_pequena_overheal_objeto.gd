...
func heal_player(vida_curada: int) -> void:
	...
	if vida_atual == GameManager.player.max_health:
		...
		if GameManager.player.player_shield > GameManager.player.max_health:
			GameManager.player.player_shield = GameManager.player.max_health
		print_debug("Adicionou " + str(vida_curada) + " ao escudo")
	...
	#Ex: se o jogador tiver 37 de vida ao curar 5 de vida ele deve ir para 40(limte de vida) e nao 42
		...
		if GameManager.player.player_shield > GameManager.player.max_health:
			GameManager.player.player_shield = GameManager.player.max_health
		print_debug("curou para vida maxima e adicionou " + str(cura_usada_para_escudo) + " de escudo")
	#Caso geral em que so adiciona somente a quantidade da pocao a vida do player
	else:
		GameManager.player.player_health += vida_curada
		print_debug("curou " + str(quantidade_cura))
	queue_free()
