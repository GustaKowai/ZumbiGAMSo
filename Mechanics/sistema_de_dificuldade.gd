...
func _process(delta):
	...
	if (int(time)%time_for_nerf == 0):
		if int(time) >= time_for_nerf*number_of_nerfs:
			...
			#buff_de_vida += 1 #Se quiser que o buff seja por tempo, descomentar isso
			#print_debug("buffados e nerfados")
			#print_debug(buff_de_vida)
			#print_debug(mobs_increase_per_minute)
	var spawn_rate = initial_spawn_rate + mobs_increase_per_minute*(time/60.0)
	spawn_rate *=wave_factor
	
	mob_spawner.mobs_per_minute = spawn_rate
	#mob_spawner.buff_de_vida = buff_de_vida #Se quiser que o buff seja por tempo, descomentar isso
